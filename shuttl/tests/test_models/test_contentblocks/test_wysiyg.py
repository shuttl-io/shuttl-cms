...
class WysiwygBaseTest(testbase.BaseTest):
     def _setUp(self):
     ...
     def test_content(self):
         content = WysiwygBlock.GetOrCreate(name="test", webpage=self.testWebpage)
         self.assertEqual(content.renderContent(None, publishing=True), "")
         content.setContent("<div>test</div>")
         self.assertEqual(content.renderContent(None, publishing=True), "<div>test</div>")
         pass